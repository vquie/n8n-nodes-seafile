...
export class Seafile implements INodeType {
	description: INodeTypeDescription = {
		...
		description: 'Access data on seafile',
		defaults: {
			name: 'seafile',
		},
		inputs: ['main'],
				...
				options: [
					{
						name: 'Create',
						value: 'create',
						description: 'Invite a user to a Seafile organization',
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		if (authenticationMethod === 'accessToken') {
			credentials = await this.getCredentials('seafileApi');
		} else {
			credentials = await this.getCredentials('seafileOAuth2Api');
		}
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				try {
					responseData = await seafileApiRequest.call(
						this,
						requestMethod,
						endpoint,
						body,
